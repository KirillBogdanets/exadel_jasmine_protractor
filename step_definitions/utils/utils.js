...
const inViewPortHelper = async (coordinates, shouldNotBe) => {
    ...
    if (shouldNotBe) {
        return expect(coordinates.y >= scrollTop && coordinates.y < scrollTop + innerHeight, "Element Is Visible.").toBe(false);
    } else {
        return expect(coordinates.y >= scrollTop && coordinates.y < scrollTop + innerHeight, "Element Is NOT Visible.").toBe(true);
    }
};
...
const textHelper = (firstValue, expected, secondValue) => {
    switch (expected) {
        case "different then":
            return expect(MemoryObject.getter(firstValue)).not.toEqual(MemoryObject.getter(secondValue), `${MemoryObject.getter(firstValue)} is equal to the ${MemoryObject.getter(secondValue)}`);
        case "equal to":
            return expect(MemoryObject.getter(firstValue)).toEqual(MemoryObject.getter(secondValue), `${MemoryObject.getter(firstValue)} is not equal to the ${MemoryObject.getter(secondValue)}`);
    }
};
...
const isElementsTextIsEqualTo = async (element, givenText, ignoringCase) => {
    ...
    if (ignoringCase) {
        return givenText.includes("$") ?
            expect(text.toLowerCase()).toEqual(MemoryObject.getter(givenText.replace("$", "")).toLowerCase()) :
            expect(text.toLowerCase()).toEqual(givenText.toLowerCase());
    } else {
        return givenText.includes("$") ?
            expect(text.toLowerCase()).toEqual(MemoryObject.getter(givenText.replace("$", ""))) :
            expect(text).toEqual(givenText);
    }
};
